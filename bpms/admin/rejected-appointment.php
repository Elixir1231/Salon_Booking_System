...
<html>
<head>
<title>BPMS || Rejected Appointment</title>
				...
				<div class="tables">
					...
					<form method="POST" action="">
						<div class="form-group">
							<input type="text" name="search" class="form-control" placeholder="Search by Appointment Number, Name, Mobile, Date, or Status" value="<?php echo isset($_POST['search']) ? $_POST['search'] : ''; ?>">
						</div>
					</form>
						<table class="table table-bordered">
							...
							<tbody>
								...
								if (isset($_POST['submit_search']) && $_POST['search'] != '') {
									...
									$search_query = " AND (tblbook.AptNumber LIKE '%$search_term%' 
														OR tbluser.FirstName LIKE '%$search_term%' 
														OR tbluser.LastName LIKE '%$search_term%' 
														OR tbluser.MobileNumber LIKE '%$search_term%' 
														OR tblbook.AptDate LIKE '%$search_term%' 
														OR tblbook.Status LIKE '%$search_term%')";
								}
								...
								$ret=mysqli_query($con,"SELECT tbluser.FirstName, tbluser.LastName, tbluser.Email, tbluser.MobileNumber, tblbook.ID AS bid, tblbook.AptNumber, tblbook.AptDate, tblbook.AptTime, tblbook.Message, tblbook.BookingDate, tblbook.Status
														...
														WHERE tblbook.Status = 'Rejected' $search_query");
								$cnt=1;
								...
								?>
									<tr>
										...
										<td width="150">
											...
											<a href="rejected-appointment.php?delid=<?php echo $row['bid'];?>" class="btn btn-danger btn-sm" onClick="return confirm('Are you sure you want to delete?')">Delete</a>
										</td>
									</tr>
								<?php 
									$cnt++;
