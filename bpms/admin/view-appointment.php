...
$ret=mysqli_query($con,"SELECT tbluser.FirstName, tbluser.LastName, tbluser.Email, tbluser.MobileNumber,
    ...
    tblbook.BookingDate, tblbook.Remark, tblbook.Status, tblbook.RemarkDate, tblbook.staff_id,
    staff_profiles.name as staff_name, staff_profiles.role as staff_role, 
    staff_profiles.specialization as staff_specialization
    FROM tblbook 
    JOIN tbluser ON tbluser.ID=tblbook.UserID 
    LEFT JOIN staff_profiles ON staff_profiles.staff_id=tblbook.staff_id 
...
<table class="table table-bordered">
    ...
    <tr>
        ...
        <td>
            <?php 
            if($row['staff_name']) {
                echo "<strong>Name:</strong> " . $row['staff_name'] . "<br>";
                echo "<strong>Role:</strong> " . $row['staff_role'] . "<br>";
                echo "<strong>Specialization:</strong> " . $row['staff_specialization'];
            } else {
                echo "No specific staff selected";
            }
            ?>
        </td>
    </tr>
...
<tr>
    ...
    <td> <?php  
        ...
        if($row['Status']=="Rescheduled") {
            echo "Rescheduled (Not Updated Yet)";
        }
    ?></td>
</tr>
</table>
						<table class="table table-bordered">
            <?php if($row['Status'] == "" || $row['Status'] == "Rescheduled"){ ?>
